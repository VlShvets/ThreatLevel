...
float Track::minErrTime = Track::DEF_MIN_ERR_TIME;
...
Results::Results(QWidget *parent) : QWidget(parent)
{
    ...
    hLayout->addWidget(new QLabel(QObject::tr("Позиционные районы\t\\\tТрассы\t")));
...
void Results::loadTable(const QVector<Area> &_area, const QVector<Track> &_track)
{
    ...
    tResults->setHorizontalHeaderLabels(QStringList() << "СКО" << "КР" << "max КР" << "БЦ" << "max БЦ" <<
                                        "КР + БЦ" << "max КР + БЦ");

    for(int i = 0; i < _area.count(); ++i)
    {
        /// Cреднеквадратическая разность времени поражения с погрешностью и точного времени поражения
        tResults->setItem(i, 0, new QTableWidgetItem(QString::number(_area.at(i).rmsDiffTime)));
        tResults->item(i, 0)->setBackgroundColor(QColor(255, 0, 0, 25));

        /// Количество ассоциированных с ПР крылатых ракет
        tResults->setItem(i, 1, new QTableWidgetItem(QString::number(_area.at(i).CMcount)));
        tResults->item(i, 1)->setBackgroundColor(QColor(255, 0, 0, 50));

        /// Максимальное количество ассоциированных с ПР крылатых ракет
        tResults->setItem(i, 2, new QTableWidgetItem(QString::number(_area.at(i).maxCMcount)));
        tResults->item(i, 2)->setBackgroundColor(QColor(255, 0, 0, 50));

        /// Количество ассоциированных с ПР баллистических целей
        tResults->setItem(i, 3, new QTableWidgetItem(QString::number(_area.at(i).BGcount)));
        tResults->item(i, 3)->setBackgroundColor(QColor(255, 0, 0, 50));

        /// Максимальное количество ассоциированных с ПР баллистических целей
        tResults->setItem(i, 4, new QTableWidgetItem(QString::number(_area.at(i).maxBGcount)));
        tResults->item(i, 4)->setBackgroundColor(QColor(255, 0, 0, 50));

        /// Количественный состав налета по ПР
        tResults->setItem(i, 5, new QTableWidgetItem(QString::number(_area.at(i).numTrack.count())));
        tResults->item(i, 5)->setBackgroundColor(QColor(255, 0, 0, 50));

        /// Максимальный количественный состав налета по ПР
        tResults->setItem(i, 6, new QTableWidgetItem(QString::number(_area.at(i).maxTrackCount)));
    ...
    if(Track::minErrTime != Track::DEF_MIN_ERR_TIME)
    {
        /// Суммарное количество крылатых ракет
        tResults->setItem(_area.count(), 1, new QTableWidgetItem(QString::number(Area::sumCMcount)));
        tResults->item(_area.count(), 1)->setBackgroundColor(QColor(255, 0, 0, 100));

        /// Максимальное суммарное количество крылатых ракет
        tResults->setItem(_area.count(), 2, new QTableWidgetItem(QString::number(Area::maxSumCMcount)));
        tResults->item(_area.count(), 2)->setBackgroundColor(QColor(255, 0, 0, 100));

        /// Суммарное количество баллистических целей
        tResults->setItem(_area.count(), 3, new QTableWidgetItem(QString::number(Area::sumBGcount)));
        tResults->item(_area.count(), 3)->setBackgroundColor(QColor(255, 0, 0, 100));

        /// Максимальное суммарное количество баллистических целей
        tResults->setItem(_area.count(), 4, new QTableWidgetItem(QString::number(Area::maxSumBGcount)));
        tResults->item(_area.count(), 4)->setBackgroundColor(QColor(255, 0, 0, 100));

        /// Количественный состав налета по всем ПР
        tResults->setItem(_area.count(), 5, new QTableWidgetItem(QString::number(Area::sumTrackCount)));
        tResults->item(_area.count(), 5)->setBackgroundColor(QColor(255, 0, 0, 100));

        /// Максимальный количественный состав налета по всем ПР
        tResults->setItem(_area.count(), 6, new QTableWidgetItem(QString::number(Area::maxSumTrackCount)));
        tResults->item(_area.count(), 6)->setBackgroundColor(QColor(255, 0, 0, 100));

        /// Минимальное время поражения ПР с погрешностью
        tResults->setItem(_area.count(), 7, new QTableWidgetItem(QString::number(Track::minErrTime)));
        tResults->item(_area.count(), 7)->setBackgroundColor(QColor(255, 0, 0, 100));
    }

    /// Максимальный количественный состав налета
    lcdMaxSumTrack->display(Area::maxSumTrackCount);
}
